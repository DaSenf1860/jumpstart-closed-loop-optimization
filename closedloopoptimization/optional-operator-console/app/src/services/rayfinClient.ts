...
import type { ClosedLoopSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<ClosedLoopSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<ClosedLoopSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<ClosedLoopSchema>({
...
export function getRayfinClient(): RayfinClient<ClosedLoopSchema> {
  if (!client) {
    throw new Error(
