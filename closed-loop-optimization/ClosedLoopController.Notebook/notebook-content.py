...
# Closed-Loop Controller — MfgRTI (Fabric Notebook)
#
# Runs one real-time pass of the closed loop: emit telemetry, run the SPSA
# optimizer, write new setpoints (auto) / recommendations (supervised) back to
# the MfgRTI KQL database. **Schedule this notebook (e.g. every 5 minutes).**
#
# Workspace: closedloopoptimization (a192508a-c0b8-46fb-8a5e-996028a36f67)
# KQL DB:    MfgRTI  |  Query URI: https://trd-8e59y4whercr4grn53.z7.kusto.fabric.microsoft.com
...
# ===== model.py =====
"""
model.py â€” Digital-twin physics + closed-loop optimizer for the injection-molding
...
from that optimum. The optimizer never sees the optimum â€” it only sees defect
rate, and must steer the setpoints toward it. Because the optimum drifts, the
loop never "finishes" â€” ideal for a live closed-loop demo.
...
# Drift of the hidden optimum (slow) â€” this is what makes the loop perpetual
...
# ===== kql_rest.py =====
"""
kql_rest.py â€” dependency-free KQL I/O using only `requests` (available in Fabric
...
# ===== loop.py =====
"""
loop.py â€” orchestration shared by the local backfill and the Fabric notebook.
...
# In Fabric, the running identity's token is used for Kusto access. Try the
# known audiences for robustness across runtime versions.
import notebookutils
def token_provider():
    for aud in ("kusto", "https://kusto.kusto.windows.net", "https://api.kusto.windows.net"):
        try:
...
for _ in range(1, 3600, 5):

    io = RestKqlIO(QUERY_URI, KQL_DB, token_provider)
    result = live_tick(io)
    print("Closed-loop tick complete:")
    for mid, s in result.items():
        print(f"  {mid}: mode={s['mode']:10s} T={s['temp']}C P={s['press']}bar")
    sleep(5)
