...
# Closed-Loop Bridge (Spark-native) — KQL <-> Rayfin SQL
#
# Applies pending operator SetpointCommands to MfgRTI (closing the supervised
# loop), then publishes live KPIs / recommendations / trend from KQL into the
# Rayfin SQL data layer the operator console reads. Uses the JVM's built-in MS
# SQL JDBC driver (via Py4J) + an Entra token — no pyodbc, no ODBC driver.
# **Schedule every 5 minutes.**
#
# Workspace: closedloopoptimization (a192508a-c0b8-46fb-8a5e-996028a36f67)
...
# ===== bridge_spark.py =====
"""
bridge_spark.py â€” Fabric/Spark-native version of the KQL <-> Rayfin SQL bridge.

Reaches the Rayfin Fabric SQL Database through the JVM's built-in Microsoft SQL
Server JDBC driver (via Py4J), using an Entra access token â€” so it needs NO
...
# --- Configuration -----------------------------------------------------------
CFG = {
    "queryUri": "https://trd-8e59y4whercr4grn53.z7.kusto.fabric.microsoft.com",
    "kqlDbName": "MfgRTI",
    "sqlServer": "xah43rtaz5pu7loxwci325jr2e-rjijfinyyd5uncs6tfqcri3pm4.database.fabric.microsoft.com",
    "sqlDatabase": "closed-loop-console-e366d2d3-c202-4379-8594-8d44c89ecacc",
}
...
# Fabric SQL endpoints (Warehouse, SQL analytics endpoint, and the native SQL
# Database TDS endpoint) accept the Power BI / Fabric token. Per the official
# notebookutils docs, getToken supports only: storage, pbi, keyvault, kusto.
def sql_token():
...
# --- Run one bridge pass -----------------------------------------------------
# SQL -> KQL applies pending operator commands; KQL -> SQL republishes live KPIs.
result = run_once(spark, CFG, kusto_token, sql_token)
print("Bridge pass complete:", result)

# Return the result as the notebook job's exit value (visible in Fabric run
# monitoring) so scheduled runs are observable without a side-channel table.
